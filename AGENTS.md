...
- **Meta / Store:** persistent **`MetaState`** in [`src/game/meta/save.ts`](src/game/meta/save.ts) (`localStorage`). Currency **Souls** (HUD + bank on title); kills grant Souls × **`getSoulIncomeMult`**. **[`StoreScene`](src/scenes/StoreScene.ts):** per-stat **◀ / ▶** rows with slot squares — spend Souls to raise a level (`tryIncrement*` in [`purchases.ts`](src/game/meta/purchases.ts)), **refund on lower** (`tryDecrement*`). Effective run stats: **`getEffectiveCharacter`** / **`getEffectiveWeapon`** and globals in [`effective.ts`](src/game/meta/effective.ts). Level caps: **`META_CAP`** in [`caps.ts`](src/game/meta/caps.ts).
...
| [`src/game/enemies/EnemyManager.ts`](src/game/enemies/EnemyManager.ts) | Spawn / move / scale / bullet hits; **no** off-screen despawn; trash spawn optional via `spawnEnemyId` + optional `trashSpawnFrequencyMult` on `EnemyUpdateOpts`; optional **lateral weave** on descent (`lateralWeaveHz` / `lateralWeaveAmplitudeT` on def); jumper evade on bullet hit; **`killEnemyAt`** emits **`ENEMY_KILLED_EVENT`** (+ `isBoss`) and **`BOSS_DEFEATED_EVENT`** for bosses |
...
| **Kill Souls (meta)** | On kill during a run: `floor(getKillRewardSouls(enemyId) × getSoulIncomeMult(meta))` (minimum **1**), added to `meta.souls` and persisted via `saveMeta`. |
...
- **Meta store:** extend caps in [`caps.ts`](src/game/meta/caps.ts); costs / `tryIncrement*` / `tryDecrement*` in [`purchases.ts`](src/game/meta/purchases.ts); per-level combat effects in [`effective.ts`](src/game/meta/effective.ts). Persisted field **`soulIncomePurchases`** (legacy saves may still carry `dollarIncomePurchases` until re-saved).  
